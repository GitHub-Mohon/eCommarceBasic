...
<div class="content-wrapper">
  ...
  <div class="row">
    <div class="col-md-8">
        ...
        <input type="text" name="name" class="form-control" id="exampleInputAcademic1" value="{{old('name',$product->name)}}">
        @error('name')
  ...
  <div class="row">
    ...
    <div class="col-md-4">
        ...
        <img src="/productHeroImages/{{$product->hero_image}}" alt="Product Hero Image Not Fund" style="max-width:100%; max-height: 100%;">
        </div>
        <div class="div">
  ...
  <div class="row">
    <div class="col-md-6">
        ...
        <input type="text" name="sku" class="form-control" id="exampleInputAcademic1" value="{{old('sku',$product->sku)}}">
        </div>
    </div>
    <div class="col-md-6">
        <div class="form-group">
  ...
  <div class="row">
    <div class="col-md-12">
        ...
        <div>
            ...
            @php
            $isChecked = $product_color->contains('color_id',$item->id);
            @endphp

            <label for=""><input type="checkbox" {{$isChecked ? 'checked' : ''}}  value="{{$item->id}}" name="color_id[]" id="color"> {{$item->name}}</label>
  ...
  <div class="row">
    <div class="col-md-6">
        ...
        <input type="text" value="{{old('price',$product ->price)}}" name="price" class="form-control" placeholder="per pis Price 20$">
        @error('price')
        <p class="text-danger">{{$message}}</p>
        @enderror
        </div>
    ...
    <div class="col-md-6">
        ...
        <input type="text" value="{{old('old_price',$product->old_price)}}" name="old_price" class="form-control" placeholder="20$">
        @error('old_price')
        <p class="text-danger">{{$message}}</p>
        @enderror
        </div>
    ...
    <div class="col-md-12">
        ...
        <table class="table">
            ...
            <tbody id="appendSection">
                ...
                <tr id="deletePrice">
                    <td>
                        <input type="text" name="size[{{$i_s}}][name]" class="form-control" value="{{$size->name}}">
                    </td>
                    <td>
                        <input type="text" name="size[{{$i_s}}][price]" class="form-control" value="{{$size->price}}">
                    </td>
                    <td>
...
@endif
  <div class="row">
    <div class="col-md-12">
        ...
        <textarea class="form-control" name="short_description" placeholder="Short Description about Product" id="shortDes">{{old('short_description',$product->short_description)}}</textarea>
    </div>
    </div>
  </div>
  <div class="row">
    <div class="col-md-12">
        ...
        <textarea class="form-control editor" name="description"  id="des">{{old('description',$product->description)}}</textarea>
    </div>
    </div>
  </div>
  ...
  <div class="row">
    <div class="col-md-12">
        ...
        <textarea class="form-control editor" name="additional_information" id="adInfo">{{old('additional_information',$product->additional_information)}}</textarea>
    </div>
    </div>
  </div>
  <div class="row">
    <div class="col-md-12">
        ...
        <textarea class="form-control editor" name="shipping_returns" placeholder="Short Description about Product" id="shortDes">{{old('shipping_returns',$product->shipping_returns)}}</textarea>
    </div>
    </div>
  </div>
  <div class="row">
                ...
                success: function (response) {
                    if (response.status) {
                        ...
                        $.each(response.sub_category, function (key, item) {
                            $('#sub_category_id').append(`<option {{($product->sub_category_id == $item->id) ? 'selected' : ''}} value="${item.id}">${item.name}</option>`);
                        });
                    }
                },
                error: function (xhr) {
